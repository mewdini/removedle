...
export const handle: Handle = async ({ event, resolve }) => {
    ...
    // save). 301 so browsers cache it, and downgrading a POST to a GET is
    // desirable here, since it means the saveScore action can never run on the
    ...
    if (event.isDataRequest || response.headers.get('x-sveltekit-page') === 'true') {
        ...
        // (Accept-Encoding, say) is widened rather than replaced, since dropping
        // one would be the same class of bug this is fixing
