<script lang="ts">
    ...
    const primaryLink = $derived(
        Object.values(links).find((v): v is string => typeof v === 'string' && v.trim() !== '')
    );
    const hasLinks = $derived(
        Object.values(links).some((v) => typeof v === 'string' && v.trim())
    );
</script>
    ...
    <div class="w-[300px] rounded-xl border border-white p-2 text-center">
        ...
        {#if errorLine?.song}
            <div class="group flex flex-col items-center">
                {#if primaryLink}
                    <a
                        class="text-xs text-white/70 italic underline decoration-dotted hover:text-white"
                        href={primaryLink}
                        target="_blank"
